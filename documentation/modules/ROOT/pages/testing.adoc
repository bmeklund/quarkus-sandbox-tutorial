...
Quarkus also provides a Dev UI, which is available in dev mode (when you start quarkus with mvn quarkus:dev) at `/q/dev` by default. It allows you to quickly visualize all the extensions currently loaded, see their status and go directly to their documentation.

To find the Dev UI endpoint, unfold the `ENDPOINTS section in the lower left corner of your IDE. If needed click the eye icon to *Show plug-in endpoints*. In the Public section select the *index-webpage(8080/https)* row. When you hover over the index-webpage row an icon will appear as below. Click in the marked icon, this opens the Dev UI

image::weblink.png[devui-page,700]
