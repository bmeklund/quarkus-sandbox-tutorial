...
In this lab guide you can either run commands from the command palette, or you can run the command manually in a terminal window. 
To open up a terminal in your IDE follow the below steps.

1. Click the hamburger menu on the left hand side

image::hamburgermenu.png[port, 700]

2. Hover over the Terminal menu item and then click ´New Terminal´ when it pops up.

image::newterminal.png[port, 700]

3. The terminal will be opened in the section below the file editor in your IDE, as below.

image::openterminal.png[port, 700]

4. Click in the terminal window and paste(allow paste if needed) alternatively type the commands manually.
...
[NOTE]
====
Before starting the dev mode, you need to append the a configuration to application.properties in the src/main/resources directory. This allows you to access the Dev UI from the cloud IDE since the Dev UI is designed for running your localhost.
====

Run the below command in a terminal window to get your sandbox domain.
[source,sh,role="copypaste"]
----
oc get route -n $(oc project -q) code-with-quarkus -o jsonpath='{.spec.host}' | sed 's/^[^.]*\.//' ; echo
----

Then add the following configuration *plus the subdomain details you got from the above command* in application.properties
...
The end result should resemble something like the below:

`%dev.quarkus.dev-ui.hosts=myuser-quarkus-workshop-index-webpage.apps.rm21.0a51.p1.openshiftapps.com`

### Live coding

Ok, lets start our application in Dev Mode. In the same terminal as you used previously to get your domain - run the below command to start your application.
...
OR Go back to the *command palette* again. Select `devfile: 05. Run Fast Jar` in the che tasks:
