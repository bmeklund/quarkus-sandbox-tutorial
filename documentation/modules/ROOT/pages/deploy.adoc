...
----
curl http://$(oc get route people -o=go-template  --template='{{ .spec.host }}')/hello/greeting/quarkus-on-openshift
----
...
If you get an error page stating the Application is not ready, try changing between http/https.
====
...
So now our app is deployed to OpenShift. You can also see it in the OpenShift Console, access it from where you started the Dev Spaces instance or go to your Developer Sandbox page. 

Login with your assigned username and password if you have been logged out:

image::ocplogin.png[login,700]

Once logged in, select the name of your project (`youruser-dev`):

image::ocpproj.png[project,700]

Select the *Workloads* tab in the project view.
This provides an overview of applications deployed to the project. You can see the single `people` deployment that we just deployed using the CLI:

image::peopledc.png[project,700]

[NOTE]
====
If you just see a list of deployments, try to change the view by clicking on the ´Graph view´ icon to the right

image::graphview.png[project,700]
====
...
Since this app is exposed to the world, a _Route_ was created which you can access using the small arrow in the upper right of the circle. Select the route link:

image::routelink.png[logs,600]

You can click on the route link, this will land you on a `Resource not found`-page but add */hello* at the end and you will get the traditional Hello response.
