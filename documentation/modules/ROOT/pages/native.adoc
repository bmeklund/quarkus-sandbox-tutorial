...
**This will take about 3 minutes to finish. Wait for it!**. In the end you should get a *BUILD SUCCESS* message.
...
Since our environment here is Linux, you can _just run it_. Run the below command
...
Or if needed, use the command palette and select **devfile: 06. Run Native App** task:
...
[source,none]
----
INFO  [io.quarkus] (main) people 1.0-SNAPSHOT native (powered by Quarkus 3.20.1.redhat-00003) started in 0.031s. Listening on: http://0.0.0.0:8081
----

That's *31* milliseconds to start up.
